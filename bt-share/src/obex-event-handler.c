...
static int __bt_get_available_int_memory(guint64 *dAvail)
{
...
static gboolean __bt_get_available_ext_memory(guint64 *available_mem_size)
{
	struct statfs fs = {0, };
...
#endif
#ifdef _TEMP_
static gchar *__bt_get_unique_file_name(gchar *storage_path, gchar *filename)
{
							...
							*server_auth_info)
{
	...
	int val = -1;
	gboolean ret = FALSE;
	unsigned guint64 available_ext_mem_size = 0;
	unsigned guint64 available_int_mem_size = 0;
